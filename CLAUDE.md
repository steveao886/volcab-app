...
**UI text is Chinese punctuation.** No ` · ` joins (0 left in UI code, from 39), and a half-width comma beside a CJK character is `，` (172 converted; the residue is AddWord's `[,，、]` split regex). A label and its key is the label plus a `.key`, not `提交 · Enter`. Study content (`meanings[].zh`, `wordNotes.json`, …) still uses half-width commas — changing it is content work, a separate decision.
...
- **`:not()` counts its argument's specificity.** `.chip:not(.chip--static):not([aria-disabled]):hover` is four classes and beat `.chip[aria-pressed='true']`'s two, so a pressed chip under the pointer — or after a tap on Android, which keeps `:hover` — drew ink text on its ink slab. Restate a state rule with `:hover` when a hover rule is built from `:not()`s.
