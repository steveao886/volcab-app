...
| 词库 | 全库列表 + 搜索(词头、英文释义、中文释义均可搜)+ 筛选(掌握状态 / 来源笔记);点击进词条详情 |
| 词条详情 | 完整词条展示 + TTS 发音 + 编辑(改释义/例句等,写回 words.json) |
...
1. 脚本解析 `Volcab.enex`(XML + 内嵌 ENML):提取各笔记中 `<b>` 标记的词/短语;对无粗体标记的笔记(如 101-103、104-106),由 AI 通读段落判定难词。
2. 清洗:去掉纯短语搭配中的常见词、还原词元、跨笔记去重;预计约 500–800 词。
3. AI 批量生成词条(音标、中英多义、情景例句、同义反义、搭配),分批产出并经 schema 校验。
4. 生成的 `words.json` 提交到 `volcab-data`;抽样人工核对若干词条质量。
5. `.enex` 解析脚本保留在 app 仓库 `scripts/` 下,便于将来再导入。
