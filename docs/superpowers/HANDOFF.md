...
记单词 PWA,SM-2 间隔重复 + 快速测验 + 词库管理,学习进度经私有仓库 `volcab-data` 跨设备同步。**六个页面全部完成并在真机日常使用**,386 个测试,类型检查 / 构建 / lint / 词库校验全绿,GitHub Actions 推送即部署。
...
| 复习 | SRS 卡片,翻面看释义/例句/同根词 + **遇见概率与一词多义的义项占比**,例句里**词头高亮**,四档打分,**翻面后可当场删词**,进度显示「还剩 N 张」。**`?mode=lapses` 是顽固词专项**:队列换成按失误次数排的前 20 个且不看到期日,其余行为完全一样 |
| 快速测试 | 六种题型轮换:看词选义、看义选词、拼写、**例句挖空、搭配填空、近义反义提示**。多义词**按义项占比加权抽释义**,次要义项不会永不出场 |
