...
记单词 PWA,SM-2 间隔重复 + 快速测验 + 词库管理,学习进度经私有仓库 `volcab-data` 跨设备同步。**六个页面全部完成并在真机日常使用**,302 个测试,类型检查 / 构建 / lint / 词库校验全绿,GitHub Actions 推送即部署。
...
| 今日 | 到期数 / 新词数 / 连续天数 / 总进度;近 7 天柱状图;同步状态角标 |
| 复习 | SRS 卡片,翻面看释义/例句/同根词,四档打分,**翻面后可当场删词**,进度显示「还剩 N 张」 |
| 快速测试 | 六种题型轮换:看词选义、看义选词、拼写、**例句挖空、搭配填空、近义反义提示** |
| 词库 | 476 行搜索(词头/中英释义,大小写不敏感)+ 状态与来源筛选 + 批量删除 |
| 词条详情 | 完整词条 + 发音 + 学习统计 + **当代遇见概率评分** + 编辑 + 删除 |
| 添加新词 | 顶部**快速收词**(只输单词,进暂存区);下方完整表单(查词典预填) |
| 统计 `/stats` | 30 天复习量、正确率趋势、断签日历、掌握分布、累计量 |
| 设置 | 每日新词数、音效开关、账号信息、导出备份 |

**生词暂存区怎么用**:`/add` 顶部输入框丢单词进去,只需要单词。攒够了在会话里让 AI 读 `staging.json`,批量生成完整词条(含 `usageScore`)后合并进 `words.json`,并**按 headword 精确移除已提升的条目**(不要清空整个文件——用户可能在此期间又加了词)。
...
**测试哲学**:UI 不写组件测试(有意决定);**但凡有真实逻辑就抽成纯函数文件并强制 TDD**。302 个测试绝大多数来自这些纯函数文件,外加 `store.test.tsx` 的 32 个编排集成测试。
...
1. **`words.json` 占 GitHub Contents API 单文件 1 MB 读取上限的 54%**(570,886 字节 / 476 词)。再加约 **400 词**就会触顶,届时该接口返回空内容,**老设备靠缓存还能用,新设备永远登录不上**。修法:`getFile` 读取时带 `Accept: application/vnd.github.raw`,上限提到 100 MB,改动集中在 `src/lib/github.ts`,十几行。
2. **三处仍无自动化测试**:30 秒防抖(需把 `PUSH_DEBOUNCE_MS` 做成可注入)、`online`/`offline`/`visibilitychange`(需在测试里覆盖 `navigator.onLine` 与 `document.visibilityState`)、演示模式。
...
5. **新添加的词几个月内不会进复习队列**:新词按词库顺序取,新词在数组末尾。符合规格,但反直觉。
